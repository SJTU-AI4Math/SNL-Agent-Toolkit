...
function applyContextLookup(tree: SnlSyntaxTree, index: Map<string, Set<string>>): void {
  const visit = (node: SnlSyntaxTree): void => {
    ...
    if (src && node.kind !== 'binder') {
      ...
      if (!declarations) node.mdata = { ...meta, srcStatus: 'dangling' };
      else if (!declarations.has(node.macro_name)) node.mdata = { ...meta, srcStatus: 'srcResolvedNoDecl' };
      else node.kind = 'bvar';
    }
...
function numeric(node: SnlSyntaxTree): boolean {
  return node.children.length === 0 && node.env_mode !== 'text' && node.env_mode !== 'block' && node.kind !== 'binder' && node.kind !== 'bvar' && /^[+-]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][+-]?\d+)?$/.test(node.macro_name.trim());
}
...
export function analyzeStructuralIndex(root: SnlSyntaxTree, macros: SnlMacroSourceLookup, entryIds: ReadonlySet<string>): SnlStructuralMetrics {
  ...
  const collect = (node: SnlSyntaxTree): void => { if (node.kind === 'binder') binders.add(node.macro_name); node.children.forEach(collect); };
  collect(root);
  ...
  const walk = (node: SnlSyntaxTree): void => {
    if (!numeric(node)) {
      ...
      const weight = catalogConstant || node.kind === 'binder' || (node.kind === 'bvar' && sourced) ? 1 : nodeWeight(node.macro_name);
...
function parseEntry(entry: EntryData): SnlSyntaxTree {
...
export async function computeEntryBareLatex(root: string, id: string): Promise<SynthResult> {
  ...
  return renderTreeAsLatex(parseEntry(entry), macros);
}
