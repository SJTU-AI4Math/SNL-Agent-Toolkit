...
Open the printed URL in a browser. The homepage shows the canonical root and Libraries using the same Library table component as the Extension Dashboard (title, slug, graph-occurrence count and graph-edge count); its rows open reading views and expose no authoring controls. Reading, SNoogL, relationship graph and common routes use the same Reader source as HTML exports. A single shared Extension `PanelHeader` contains the current title, navigation, refresh, read-only status, preferences and language controls instead of stacked host toolbars. Saving workspace files automatically updates the Library list and current reading view through a shared filesystem watcher and same-origin change stream; the current URL/history are retained. Save bursts are coalesced and reconnect catches missed changes. Connection or watcher failures are visible, and manual Refresh remains available. Transient graph camera/disclosure state is not guaranteed across a rebuilt snapshot. No editor or data-source switching is included in this first version. Source/Monaco viewing is not yet connected in this local host (existing HTML export source viewing is unchanged). Static HTML exports remain frozen and must be regenerated to reflect changed files.
