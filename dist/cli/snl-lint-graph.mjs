...
`);
  ...
  const root = path2.resolve(String(parsed.flags.root));
...
`);
  ...
  const targets = [...parsed.positional.map((p) => path2.resolve(p))];
  ...
  if (targets.length === 0) {
    ...
    if (targets.length === 0) {
      process.stderr.write(
        `No graph.json files found under ${path2.join(root, ".SNL_Doc/libraries")}.
`
      );
      return 2;
    }
  ...
  for (const abs of targets) {
    let raw;
    try {
      const text = await fs2.readFile(abs, "utf8");
      raw = JSON.parse(text);
    } catch (err) {
      reports.push({
        file: abs,
