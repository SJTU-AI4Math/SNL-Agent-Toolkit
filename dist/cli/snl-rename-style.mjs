...
async function isCanonicalAcquisition(handle, lockPath, parent) {
  try {
    const currentParent = await lstat(path3.dirname(lockPath), { bigint: true });
...
async function acquireLock(workspaceRoot, purpose) {
  ...
  const parent = await lstat(path3.dirname(lockPath), { bigint: true });
...
async function styleRenameUnlocked(root, packageId, macroId, oldStyle, newStyle, dryRun, expectedPlan) {
  ...
  const plan = fingerprintPlan({
    ...
    oldStyle,
    newStyle,
    occurrences: occurrences.sort((a4, b3) => a4.file.localeCompare(b3.file) || a4.path.localeCompare(b3.path) || (a4.offset ?? -1) - (b3.offset ?? -1)),
  ...
  try {
    await Promise.all(replacements.map(async (file) => {
      await fs3.writeFile(file.temp, file.next, { encoding: "utf8", mode: file.mode, flag: "wx" });
    ...
    await Promise.all(replacements.map((file) => fs3.rm(file.temp, { force: true })));
  }
  return plan;
}
...
function targetMacroResolves(files, packageId, macroId) {
  ...
  for (const file of files) {
    ...
    if (file.relPath.startsWith("term_macros/") && isRecord2(data?.macros) && Object.prototype.hasOwnProperty.call(data.macros, macroId)) packages.push(path4.posix.basename(file.relPath, ".json"));
  }
  const candidates = [...new Set(packages)].filter((id) => !active || active.has(id)).sort((a4, b3) => `${a4}.json`.localeCompare(`${b3}.json`));
...
async function main() {
  ...
  try {
    const plan = await renameStyle(path5.resolve(String(parsed.flags.root)), packageId, macroId, oldStyle, newStyle, { dryRun });
