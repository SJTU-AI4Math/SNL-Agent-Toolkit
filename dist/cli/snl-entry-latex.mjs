...
function d(e2) {
  ...
  let t4 = 0, n3 = u(e2, t4, true);
  if (n3 === 0) return false;
  for (t4 += n3; t4 < e2.length; ) {
    if (n3 = u(e2, t4, false), n3 === 0) return false;
    t4 += n3;
  }
  return true;
}
var p = 100;
function m(e2, t4, n3 = false) {
  return {
    positional_arity: e2,
    variadic: t4,
...
function v(e2) {
  let t4 = e2.replace(/\\#/g, "ESCAPED_HASH"), n3 = -1;
  for (let e3 of t4.matchAll(/#(\d{1,2})(?!\d)/g)) n3 = Math.max(n3, Number(e3[1]));
  return m(n3 + 1, /#\*/.test(t4), /#\d{3,}/.test(t4));
}
...
function x(e2, t4) {
  let n3 = e2.length - t4;
  if (n3 >= 2 && e2[t4] === "`") {
    let n4 = e2.indexOf("`", t4 + 1);
    if (n4 < 0) throw new b("Unclosed ` delimiter", t4);
    return {
      token: {
        type: "BACKTICK_DELIMITED",
        value: e2.slice(t4 + 1, n4),
        position: t4
      },
      next: n4 + 1
    };
  }
  if (n3 >= 4 && e2[t4] === "$" && e2[t4 + 1] === "$") {
    let n4 = e2.indexOf("$$", t4 + 2);
    if (n4 < 0) throw new b("Unclosed $$ delimiter", t4);
    return {
      token: {
        type: "DOLLAR2_DELIMITED",
        value: e2.slice(t4 + 2, n4),
        position: t4
      },
      next: n4 + 2
    };
...
function T(e2, t4 = []) {
  e2.env_mode && (e2.temporary_source = e2.macro_name, e2.macro_name = t4.length === 0 ? "#" : `#${t4.join(".")}`), e2.binder_explicit && e2.binder_name === void 0 && (e2.binder_name = e2.temporary_source ?? e2.macro_name), e2.children.forEach((e3, n3) => T(e3, [...t4, n3]));
}
function E(e2) {
  try {
    return {
...
function k(e2, t4) {
  return e2.length === t4.length && e2.every((e3, n3) => e3 === t4[n3]);
}
function A(e2, t4) {
  let n3 = 0;
  for (; n3 < e2.length && n3 < t4.length && e2[n3] === t4[n3]; ) n3 += 1;
  return n3;
}
function j(e2, t4, n3) {
  return e2.filter((e3) => !n3 || e3.order < t4.order).sort((e3, n4) => A(n4.path, t4.path) - A(e3.path, t4.path) || n4.order - e3.order)[0];
}
function M(e2) {
  return e2.temporary_source ?? e2.macro_name;
}
function N(e2, t4) {
  let n3 = O(e2), r3 = [], i4 = [], a4 = 0, o4 = (e3, n4) => {
    i4.push({
      node: e3,
      path: n4,
      order: a4++
    }), e3.scope = void 0;
    let s5 = e3.env_mode ? void 0 : t4[e3.macro_name], c3 = n4.length === 0 && e3.env_mode === "text", l3 = s5?.kind === "sub";
    ...
    e3.children.forEach((e4, t5) => o4(e4, [...n4, t5]));
  };
  o4(n3, []);
...
function o2(e2) {
  ...
  let n3 = [...t4.matchAll(/([a-z][a-z0-9-]*)\s*\(/gi)];
...
function s2(e2, t4) {
  ...
  for (let n3 of [
    ...
    e2.background,
    e2.border
  ]) if (!o2(n3)) throw Error(`table.css.${t4} contains an invalid CSS color`);
  return {
    color: e2.color,
...
function i2(e2) {
  ...
  let n3 = t4.values, r3 = Object.keys(n3);
  return r3.length > 0 && Object.prototype.hasOwnProperty.call(n3, t4.default_language) && typeof n3[t4.default_language] == "string" && r3.every((e3) => typeof n3[e3] == "string");
}
function a2(e2) {
  if (!e2 || typeof e2 != "object" || Array.isArray(e2)) return false;
  let t4 = e2;
...
function j2(e2) {
  ...
  let n3 = t4.values;
  return !Object.prototype.hasOwnProperty.call(n3, t4.default_language) || Object.keys(n3).length === 0 || !Object.values(n3).every(k2) ? null : Object.values(n3);
}
function M2(t4) {
  let n3 = v(t4.body);
...
function F(t4) {
  ...
  for (let r3 of Object.values(t4)) {
    ...
    for (let r4 of t5.styles) {
      ...
      if (typeof a4.style_name != "string" || !d(a4.style_name) || !o3(a4.tags) || !s4 || N2.some((e2) => e2 in a4) || Object.keys(a4).some((e2) => !P.has(e2)) || new Set(s4.map(M2)).size !== 1 || s4.some((n3) => {
        let r5 = v(n3.body);
...
var K = 256;
function q(e2, t4) {
  return e2.reduce((n3, r3, i4) => i4 === 0 ? r3 : `${n3}${e2[i4 - 1] !== "" && r3 !== "" ? `,${t4}` : ","}${r3}`, "");
}
var J = class {
  indentSpaces;
  ...
  formatNode(e2, t4, n3) {
    ...
    return `${r3}(
${e2.children.map((e3) => `${i4}${this.formatNode(e3, t4 + 1, n3)}`).join(",\n")}
${" ".repeat(this.indentSpaces * t4)})`;
  }
  formatNodeHead(e2) {
    let t4 = e2.binder_explicit ? "@" : "", n3, r3 = e2.temporary_source ?? e2.macro_name;
  ...
  assertIntegerInRange(e2, t4, n3) {
    if (!Number.isSafeInteger(e2) || e2 < 0 || e2 > n3) throw RangeError(`${t4} must be a non-negative integer no greater than ${n3}`);
  }
  parenthesisDepth(e2) {
    let t4 = -1;
    for (let n3 of e2.children) t4 = Math.max(t4, this.parenthesisDepth(n3));
    return t4 + 1;
  }
};
...
function latexToText(input, notes) {
  ...
  for (let i4 = 0; i4 < 5; i4++) {
    const next = s4.replace(wrapperRe, (_, inner) => inner);
    if (next === s4) break;
    s4 = next;
  }
  const fracRe = /\\frac\s*\{([^{}]*)\}\s*\{([^{}]*)\}/g;
  for (let i4 = 0; i4 < 5; i4++) {
    const next = s4.replace(fracRe, (_, a4, b3) => `(${a4})/(${b3})`);
    if (next === s4) break;
    s4 = next;
  }
  s4 = s4.replace(
...
function fillTemplate(template, values) {
  ...
  out = out.replace(/#(\d{1,2})/g, (_, d3) => {
    const v2 = values[`child${Number(d3)}`];
    return v2 === void 0 ? `#${d3}` : v2;
  });
  out = out.replace(/#\*/g, () => {
